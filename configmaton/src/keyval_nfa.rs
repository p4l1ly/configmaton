...
fn bytes_as_string(bytes: &[u8]) -> String {
    bytes
        .iter()
        .map(
            |b| if b.is_ascii_graphic() { char::from(*b).to_string() } else { format!("\\{}", b) },
        )
        .collect()
}
...
fn fmte(exts: &Vec<Vec<u8>>, get_olds: &Vec<Vec<u8>>) -> String {
    exts.iter()
        .map(|ext| bytes_as_string(ext))
        .chain(get_olds.iter().map(|old| format!("GetOld({})", bytes_as_string(old))))
        .collect::<Vec<_>>()
        .join(", ")
        .replace("\\", "\\\\")
        .replace("\"", "\\\"")
    ...
    match bdd {
        BddOrigin::Leaf(target) => {
            ...
            write(format!(
                ...
                tix,
                tix,
                fmte(&target.exts, &target.get_olds)
            ));
        ...
        _ => {
            ...
            *bix += 1;
            let pos =
                visited.entry(pos as *const _).or_insert_with(|| to_dot(pos, bix, tix, write));
            write(format!(
                "  {} -> {} [ color=green{} ]\n",
                me,
                pos,
            ...
            let neg =
                visited.entry(neg as *const _).or_insert_with(|| to_dot(neg, bix, tix, write));
            write(format!(
                "  {} -> {} [ color=red{} ]\n",
                me,
                neg,
...
impl Parser {
    ...
    fn parse_label(&mut self, label_name: &str, label_body: &Cmd) -> LeafOrigin {
        ...
        if self.parsing_labels.contains(label_name) {
            ...
            return LeafOrigin { states: vec![], get_olds: vec![], exts: vec![] };
        }
    ...
    fn parse_match(&mut self, match_: Match) -> LeafOrigin {
            ...
            match_.when[..guard_count - 1].into_iter().zip(dfa_ixs.iter()).rev()
        {
            ...
            let else_ = LeafOrigin {
                exts: vec![],
                get_olds: vec![],
                states: vec![state_ix + guard_count],
            };
            self.states.push(StateOrigin {
        ...
        {
            let state_ix = self.states.len();
            let else_ = LeafOrigin { exts: vec![], get_olds: vec![], states: vec![state_ix] };
            self.states.push(StateOrigin {
                transitions: vec![TranOrigin {
                    key: key.clone().into_bytes(),
                    dfa_inits: vec![dfa_state_ix.0],
...
impl<'de> Visitor<'de> for CmdVisitor {
        ...
        V: MapAccess<'de>,
    {
        ...
        match (when, goto, label) {
            (Some(when), None, None) => {
                let run =
                    run.unwrap_or_else(|| vec![]).into_iter().map(|s| s.into_bytes()).collect();
                let then = then.unwrap_or_else(|| vec![]);
                Ok(Cmd::Match(Match { when, run, then }))
            }
            (None, Some(goto), None) => Ok(Cmd::Goto(goto)),
...
mod tests {
    ...
    fn config_to_automaton_simple() {
        ...
        sim.read(b"foo", b"a", |x| match x {
            b"foo" => Some(b"a"),
            _ => None,
        });
        assert!(sim.exts.is_empty());
        sim.read(b"foo", b"b", |x| match x {
            b"foo" => Some(b"b"),
            _ => None,
        });
        assert!(sim.exts.is_empty());
        sim.read(b"bar", b"b", |x| match x {
            b"foo" => Some(b"b"),
            b"bar" => Some(b"b"),
            _ => None,
        });
    ...
    fn config_to_automaton_simplest() {
        // read and parse file tests/config.json
        let config: Vec<Cmd> =
            serde_json::from_str(r#"[{"when": {"foo": "a"}, "run": ["bar"]}]"#).unwrap();
    ...
    fn test_goto_label_comprehensive() {
        ...
        let mut sim = Simulation::new(aut, |_| None);
        sim.read(b"test_type", b"basic_goto", |x| match x {
        ...
        let mut sim = Simulation::new(aut, |_| None);
        sim.read(b"test_type", b"forward_ref", |x| match x {
        ...
        // Test cycle A -> B
        let mut sim = Simulation::new(aut, |_| None);
        sim.read(b"test_type", b"cycle_a", |x| match x {
        ...
        let mut sim = Simulation::new(aut, |_| None);
        sim.read(b"test_type", b"nested_goto", |x| match x {
    ...
    #[test]
    fn goto() {
        let _config: Vec<Cmd> = serde_json::from_str(
            r#"[
            {
                "when": { "foo": "baz" },
                "then": [
