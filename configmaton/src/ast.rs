...
fn parse_ext_ast(ext: &ast::Ast) -> Ast {
    match ext {
        ...
        ast::Ast::Repetition(a) => Ast::Repetition(Box::new(parse_ext_ast(&a.ast))),
        ast::Ast::Group(a) => parse_ext_ast(&a.ast),
        ast::Ast::ClassBracketed(x) => {
            if x.negated {
                panic!("negated class bracketed not supported");
            }
            match &x.kind {
                ast::ClassSet::Item(item) => parse_ext_class_set_item(item),
                _ => {
...
fn parse_ext_class_set_item(item: &ast::ClassSetItem) -> Ast {
    match item {
        ast::ClassSetItem::Range(range) => Ast::Range(range.start.c as u8, range.end.c as u8),
        ...
        ast::ClassSetItem::Union(union) => {
            ...
            for child in union.items[1..].iter() {
                result =
                    Ast::Alternation(Box::new(result), Box::new(parse_ext_class_set_item(child)));
            }
            result
        }
        _ => {
...
mod tests {
    ...
    fn test_parse_regex() {
        ...
        assert_eq!(
            ast,
            Ast::Concatenation(Box::new(Ast::Range(b'a', b'a')), Box::new(Ast::Range(b'b', b'b')))
        );
            ...
            Ast::Concatenation(
                ...
                Box::new(Ast::Alternation(
                    Box::new(Ast::Alternation(
                        ...
                        Box::new(Ast::Repetition(Box::new(Ast::Concatenation(
                            Box::new(Ast::Alternation(
